...
func _ready():
	player.inventory.connect("items_changed", self, "_update_items")
...
func _update_items():
	for button in $Box/Scroll/Items.get_children():
		var item = player.inventory.items[button.item_id]
...
func _on_about_to_show():
	...
	for i in player.inventory.items.size():
		...
		if item and item.PRICE > 0 and not item is ToolItem:
...
func _on_item_pressed(button):
	...
	Global.current_scene.market.sell_item(player.inventory.remove_item_id(button.item_id))
...
func _on_item_focus_entered(button):
	...
	$Box/PriceLabel.text = "Price: %sG" % player.inventory.items[button.item_id].PRICE
